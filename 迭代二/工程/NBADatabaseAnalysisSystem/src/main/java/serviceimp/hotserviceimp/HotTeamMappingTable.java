...
public class HotTeamMappingTable extends MappingTable {
	...
	public HotTeamMappingTable() {
		...
		addMap("三分%", "round(sum(pmp.three_point_shootings)/sum(pmp.three_point_shots),1)");
		addMap("%", "round(pmp.shootings)/sum(pmp.shots),1)");
		addMap("罚球%", "round(pmp.free_throw_shootings)/sum(pmp.free_throw_shots),1)");
	}
